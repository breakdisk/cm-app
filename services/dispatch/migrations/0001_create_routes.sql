...
CREATE TABLE dispatch.route_stops (
    id                  UUID        PRIMARY KEY DEFAULT uuid_generate_v4(),
...
CREATE INDEX idx_routes_tenant_driver  ON dispatch.routes (tenant_id, driver_id);
CREATE INDEX idx_routes_status         ON dispatch.routes (tenant_id, status);
CREATE INDEX idx_stops_route_sequence  ON dispatch.route_stops (route_id, sequence);
CREATE INDEX idx_stops_point           ON dispatch.route_stops USING GIST (point);
