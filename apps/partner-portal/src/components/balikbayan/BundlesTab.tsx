...
function exportCsv(bundles: BoxBundle[]) {
  const header = "id,name,description,items,price_usd,valid_for,notes";
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 px-5 py-4 border-b border-glass-border">
        <div>
          ...
          <p className="text-2xs font-mono text-white/30 mt-0.5">
            Pre-packaged multi-box deals · fixed USD price · assigned to sea, air, or both services
          </p>
        </div>
        {!editing && bundles.length > 0 && (
          <button
            onClick={() => exportCsv(bundles)}
          ...
          <div key={b.id} className="px-5 py-4 hover:bg-glass-100/20 transition-colors">
            {editing ? (
              ...
              <div className="flex flex-col gap-3">
                {/* Row 1: name / description / price / delete */}
                <div className="grid grid-cols-[1fr_1fr_100px_36px] gap-2 items-start">
                  <input
                    value={b.name}
                    onChange={(e) => patchBundle(bIdx, "name", e.target.value)}
                    placeholder="Bundle name"
                    className={inputCls}
                  />
                  <input
                    value={b.description}
                    onChange={(e) => patchBundle(bIdx, "description", e.target.value)}
                    placeholder="Short description"
                    className={inputCls}
                  />
                  <div className="relative">
                    <span className="absolute left-2 top-1/2 -translate-y-1/2 text-xs text-white/30 font-mono">$</span>
                    <input
                      type="number"
                      min={0}
                      step={1}
                ...
                <div className="grid grid-cols-[120px_1fr] gap-2">
                  <select
                    value={b.valid_for}
                    onChange={(e) => patchBundle(bIdx, "valid_for", e.target.value as BundleScope)}
                    className={inputCls}
                  >
                  ...
                  <input
                    value={b.notes}
                    onChange={(e) => patchBundle(bIdx, "notes", e.target.value)}
                    placeholder="Additional notes (promo, expiry, eligibility…)"
                    className={inputCls}
                  />
                </div>
                  ...
                  <div className="flex flex-col gap-1.5">
                    {b.items.map((item, iIdx) => (
                      <div key={iIdx} className="flex items-center gap-2">
                        <select
                          value={item.size_id}
                          onChange={(e) => patchItem(bIdx, iIdx, "size_id", e.target.value)}
              ...
              <div className="flex items-start justify-between gap-4">
                <div className="flex items-start gap-3 min-w-0">
                  ...
                  <div className="min-w-0">
                    ...
                    {b.description && (
                      <p className="text-xs text-white/50 mb-2">{b.description}</p>
                    )}
                    <div className="flex flex-wrap gap-2">
                      {b.items.map((item, i) => (
                        <span
                          key={i}
                          className="rounded-full border border-glass-border px-2.5 py-0.5 text-2xs font-mono text-white/50"
                        >
                          {item.quantity}× {sizeNameOf(boxSizes, item.size_id)}
                        </span>
                      ))}
                      {b.items.length === 0 && (
                        <span className="text-2xs font-mono text-white/20">No items</span>
                      )}
                    </div>
                    {b.notes && (
                      <p className="text-2xs font-mono text-white/30 mt-1.5">{b.notes}</p>
                    )}
                  </div>
                </div>
                <div className="flex-shrink-0 text-right">
                  <p className="text-xl font-bold font-mono text-cyan-neon">${b.price_usd}</p>
                  <p className="text-2xs font-mono text-white/30">USD / bundle</p>
                </div>
              </div>
            )}
          </div>
