...
function emptyZone(): AirCargoZone {
  return { zone_name: "", origins: "", currency: "USD", rate_per_kg_usd: 0, distance_surcharge_per_km: 0, transit_days: "" };
}

function exportCsv(zones: AirCargoZone[], fixed: AirCargoFixed) {
  const zoneHeader = "zone_name,currency,origins,rate_per_kg,distance_surcharge_per_km,transit_days";
  const zoneRows   = zones.map((z) =>
    [`"${z.zone_name}"`, z.currency ?? "USD", `"${z.origins}"`, z.rate_per_kg_usd, z.distance_surcharge_per_km, z.transit_days].join(",")
  );
  const cur = fixed.currency ?? "USD";
  const fixedBlock = [
    "", "FIXED CHARGES", `currency,${cur}`,
    `AWB Fee,${fmtCurrency(fixed.awb_fee_usd, cur)}`,
    `Fuel Surcharge,${(fixed.fuel_surcharge_pct * 100).toFixed(0)}%`,
    `THC,${fmtCurrency(fixed.thc_usd, cur)}`,
    `PH Customs,${fmtCurrency(fixed.customs_clearance_usd, cur)}`,
...
export function AirCargoTab({ zones, fixed, editing, onZonesChange, onFixedChange }: Props) {
  ...
  function patchFixed(key: keyof AirCargoFixed, value: string | number) {
    ...
    <div className="flex flex-col gap-4">
      ...
      <GlassCard glow="purple" accent>
        <p className="text-xs font-mono text-purple-plasma">
          FORMULA: Total Air Freight = (Chargeable Weight × Zone Rate/kg) + (Distance km × Dist. Surcharge)
          + AWB {fmtCurrency(fixed.awb_fee_usd, fixedCur)} + FSC {(fixed.fuel_surcharge_pct * 100).toFixed(0)}%
          + THC {fmtCurrency(fixed.thc_usd, fixedCur)}
        </p>
      ...
      <GlassCard padding="none">
        <div className="flex items-center justify-between px-5 py-4 border-b border-glass-border">
          <div>
            ...
            <p className="text-2xs font-mono text-white/30 mt-0.5">FROM origin country → TO NAIA / Mactan / Clark · per-zone currency</p>
          </div>
          {!editing && (
        ...
        {/* Headers: Zone | Origins | Currency | Rate/kg | Dist Surcharge | Transit | Delete */}
        <div className="grid grid-cols-[150px_1fr_70px_110px_130px_90px_36px] gap-2 px-5 py-2.5 border-b border-glass-border">
          {["Zone", "Origin Countries / Regions", "Currency", "Rate / kg", "Dist. Surcharge / km", "Transit", ""].map((h, i) => (
            <span key={i} className="text-2xs font-mono text-white/30 uppercase tracking-wider">{h}</span>
          ))}
        </div>

        {zones.map((z, idx) => {
          const cur = z.currency ?? "USD";
          return (
            <div key={idx} className="grid grid-cols-[150px_1fr_70px_110px_130px_90px_36px] gap-2 items-center px-5 py-2.5 border-b border-glass-border/40 hover:bg-glass-100/40 transition-colors">
              {editing ? (
              ...
              {editing ? (
                <CurrencySelect value={cur} onChange={(code) => patchZone(idx, "currency", code)} className={selectCls} />
              ) : (
              ...
              {editing ? (
                ...
                <span className="text-xs font-bold font-mono text-cyan-neon">{fmtCurrency(z.rate_per_kg_usd, cur)}/kg</span>
              )}
              ...
              {editing ? (
                ...
                <span className="text-xs font-mono text-white/60">{fmtCurrency(z.distance_surcharge_per_km, cur)}/km</span>
              )}
        ...
        <div className="flex items-center justify-between px-5 py-4 border-b border-glass-border">
          ...
          {editing && (
            <div className="flex items-center gap-2">
              <span className="text-2xs font-mono text-white/40">Fixed charges currency:</span>
              <CurrencySelect
                value={fixedCur}
                onChange={(code) => patchFixed("currency", code)}
                className="rounded-md border border-glass-border bg-glass-100 px-2 py-1.5 text-xs text-white outline-none focus:border-purple-plasma/60 w-24"
              />
            </div>
          )}
          {!editing && (
            <span className="text-2xs font-mono font-bold text-white/50 border border-glass-border/40 rounded px-2 py-0.5">{fixedCur}</span>
          )}
        </div>

        <div className="grid grid-cols-2 gap-0 divide-y divide-glass-border/40 sm:grid-cols-3">
          {([
            { label: "AWB Fee",             key: "awb_fee_usd",           fmt: (v: number) => fmtCurrency(v, fixedCur),          step: 1    },
            { label: "Fuel Surcharge (FSC)",key: "fuel_surcharge_pct",    fmt: (v: number) => `${(v*100).toFixed(0)}%`,          step: 0.01 },
            { label: "THC (Origin Airport)",key: "thc_usd",               fmt: (v: number) => fmtCurrency(v, fixedCur),          step: 1    },
            { label: "PH Customs Clearance",key: "customs_clearance_usd", fmt: (v: number) => fmtCurrency(v, fixedCur),          step: 1    },
