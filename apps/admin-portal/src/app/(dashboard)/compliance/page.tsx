...
export default function CompliancePage() {
  ...
  // Incrementing this key forces DocumentDetailPanel to re-fetch its profile
  // after an approve / reject / suspend / reinstate action.
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      ...
      toast.error((err as Error).message ?? "Failed to load compliance data");
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  // ── Document actions ────────────────────────────────────────────────────────
  // The panel calls these callbacks after the user confirms — the parent owns
  // the API call so it can update the queue list and show a toast. The
  // panelRefreshKey bump causes the panel to re-fetch its local detail view.
